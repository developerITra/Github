...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queAttyMilestone6Mgr"
...
Dim cntr As Integer
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryqueueAttymilestone6Mgr", dbOpenDynaset, dbSeeChanges)
...
'Set rstJnl = CurrentDb.OpenRecordset("Select * FROM journal where filenumber=" & lstFiles, dbOpenDynaset, dbSeeChanges)
    ...
    strSQLJournal = "Insert into Journal (FileNumber,JournalDate,Who,Info,Color) Values(Forms!queAttyMilestone6Mgr!lstFiles,Now,GetFullName(),'" & strinfo & "',1 )"
