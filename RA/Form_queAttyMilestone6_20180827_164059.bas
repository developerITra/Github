...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queAttyMilestone6"
...
With rstqueue
.Edit
!AttyMilestone6Reject = False
!AttyMilestone6 = Now
!AttyMilestone6Approver = GetStaffID
.Update
End With
...
Dim cntr As Integer
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryqueueAttymilestone6", dbOpenDynaset, dbSeeChanges)
...
'Set rstJnl = CurrentDb.OpenRecordset("Select * FROM journal where filenumber=" & lstFiles, dbOpenDynaset, dbSeeChanges)
    ...
    strSQLJournal = "Insert into Journal (FileNumber,JournalDate,Who,Info,Color) Values(Forms!queAttyMilestone6!lstFiles,Now,GetFullName(),'" & strinfo & "',2 )"
...
'Set rstJnl = CurrentDb.OpenRecordset("Select * FROM journal where filenumber=" & lstFiles, dbOpenDynaset, dbSeeChanges)
    ...
    strinfo = "Pre Sale Review- Rejected because " & InputBox("Enter Reason for Rejection", "Attorney Review")
    strinfo = Replace(strinfo, "'", "''")
    strSQLJournal = "Insert into Journal (FileNumber,JournalDate,Who,Info,Color) Values(Forms!queAttyMilestone6!lstFiles,Now,GetFullName(),'" & strinfo & "',4 )"
    DoCmd.RunSQL strSQLJournal
    DoCmd.SetWarnings True
...
'2/7/14
    ...
    strinfo = Replace(strinfo, "'", "''")
    strSQLJournal = "Insert into Journal (FileNumber,JournalDate,Who,Info,Color) Values(Forms!queAttyMilestone6!lstFiles,Now,GetFullName(),'" & strinfo & "',1)"
    DoCmd.RunSQL strSQLJournal
    DoCmd.SetWarnings True
