...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_sfrmDCComplaintNew"
...
Private Sub ServiceCancelled_AfterUpdate()
    If Not IsNull(ServiceCancelled) Then
    AddStatus FileNumber, ServiceCancelled, "Service cancelled with process server"
    
 'Forms!ForeclosureDetails!sfrmStatus.Requery
    
 DoCmd.SetWarnings False
    strinfo = "Service cancelled with process server"
    strinfo = Replace(strinfo, "'", "''")
    strSQLJournal = "Insert into Journal (FileNumber,JournalDate,Who,Info,Color) Values(FileNumber,Now,GetFullName(),'" & strinfo & "',1 )"
    DoCmd.RunSQL strSQLJournal
    DoCmd.SetWarnings True
...
Private Sub ServiceCancelled_DblClick(Cancel As Integer)
    If FileReadOnly Or EditDispute Then
    DoCmd.CancelEvent
Else
    ServiceCancelled = Now()
    Call ServiceCancelled_AfterUpdate
End If
...
Private Sub txtCompiledExhibits_AfterUpdate()

If Not IsNull(txtCompiledExhibits) Then
 AddStatus FileNumber, ApprovedDraft, "Compiled Exhiibits"

Else
 AddStatus FileNumber, Now(), "Removed Compiled Exhiibits"
End If

End Sub

Private Sub txtCompiledExhibits_DblClick(Cancel As Integer)
If FileReadOnly Or EditDispute Then
   DoCmd.CancelEvent
Else
    txtCompiledExhibits = Now()
    Call txtCompiledExhibits_AfterUpdate
End If
End Sub
