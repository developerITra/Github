...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queAttyMilestone1_25Mgr"
...
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM wizardqueuestats where filenumber= " & Forms!queAttyMilestone1_25Mgr!lstFiles & " and current=true", dbOpenDynaset, dbSeeChanges)
...
Dim cntr As Integer
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryqueueAttyMilestone1_25Mgr", dbOpenDynaset, dbSeeChanges)
...
strinfo = "Demand Review- Manager corrections made:  " & mgrInput
