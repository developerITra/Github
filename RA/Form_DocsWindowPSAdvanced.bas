...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_DocsWindowPSAdvanced"
...
Dim i As Integer
 DoCmd.SetWarnings False
    For i = 0 To lstDocs.ListCount
        ...
        JurText = "Complete the hold of PS Advanced Costs for " & Forms!DocsWindowPSAdvanced.lstDocs.Column(3, i)
         DoCmd.RunSQL ("UPDATE DocIndex set Hold = '' WHERE DocID = " & Forms!DocsWindowPSAdvanced.lstDocs.Column(0, i))
         DoCmd.RunSQL ("UPDATE Accou_PSAdvancedCostsPackageQueue set Hold = '',Dismissed = True , MangerQ = False  WHERE DocIndexID = " & Forms!DocsWindowPSAdvanced.lstDocs.Column(0, i))
...
Forms![DocsWindowPSAdvanced]!lstDocs.ColumnCount = 6
Forms![DocsWindowPSAdvanced]!lstDocs.ColumnWidths = "0 in; 0.4 in; 0.75 in; 3 in; 0 in ;0.3 in "
Forms![DocsWindowPSAdvanced]!lstDocs.RowSource = "SELECT DocID, Initials, Format(Datestamp,""mm/dd/yyyy"") AS [Date Entered], Filespec as [File Name], [doctitleid] AS DocType , Hold,DocTitleID FROM DocIndex LEFT JOIN Staff ON DocIndex.StaffID=Staff.ID WHERE FileNumber=" & FileNumber & " AND DocGroup='B' AND Hold = 'H' AND DocTitleID = 1566 AND Filespec IS NOT NULL and DeleteDate is null"
Forms![DocsWindowPSAdvanced]!lstDocs.Requery
