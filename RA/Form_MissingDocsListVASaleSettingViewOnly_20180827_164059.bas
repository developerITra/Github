...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_MissingDocsListVASaleSettingViewOnly"
...
Set rstdocs = CurrentDb.OpenRecordset("Select * FROM VAsalesettingDocsNeeded where filenumber=" & FileNbr & " AND DOCID=" & lstFiles.Value, dbOpenDynaset, dbSeeChanges)
...
Set rstdocs = CurrentDb.OpenRecordset("Select * FROM VAsalesettingDocsNeeded where filenumber=" & FileNbr, dbOpenDynaset, dbSeeChanges)
...
'!JournalDate = Now
'!Who = GetFullName
'!Info = DocName & "was manually removed from the VA sale setting waiting list of outstanding items"
'.Update
'.Close
'End With

DoCmd.SetWarnings False
strinfo = DocName & "was manually removed from the VA sale setting waiting list of outstanding items"
...
With rstqueue
.Edit
!VASaleSettingDocsRecdFlag = True
.Update
End With
