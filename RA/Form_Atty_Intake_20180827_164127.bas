...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_Atty_Intake"
...
newfilename = "Atty Intake Cover Letter " & " " & Format$(Now(), "yyyymmdd hhnnss")
...
DoCmd.OutputTo acOutputForm, "Atty_Intake", acFormatPDF, DocLocation & DocBucket(CaseFile) & "\" & CaseFile & "\" & newfilename & ".pdf", False, "", 0
...
strinfo = "Sent to Attorney Intake Review."
'Else
...
Do
    ...
    For Each F In Forms
        Select Case F.Name
            Case "Main", "queIntakeWaiting", "queintake" '  leave these forms open
            Case Else
