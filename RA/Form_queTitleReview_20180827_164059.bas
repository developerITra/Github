...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queTitleReview"
...
DoCmd.SetWarnings False
    DoCmd.OutputTo acOutputQuery, "qryQueueTiteReview", acFormatXLS, TemplatePath & "Title Review Queue info.xlt", True
...
Dim rstqueue As Recordset
AddToList (lstFiles)
TitleReviewCallFromQueue lstFiles
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM wizardqueuestats where filenumber=" & lstFiles & " AND current=true", dbOpenDynaset, dbSeeChanges)

With rstqueue
.Edit
!TitleReviewLastReviewBy = GetStaffID
!TitleReviewLastReview = Date
.Update
End With
Set rstqueue = Nothing
End Sub
...
'Count = DCount("File", "qryQueueTiteReview")
'Debug.Print Count
'Set rstqueue = CurrentDb.OpenRecordset("select count(*) as ct from qryQueueTiteReview")
...
QueueCount = DCount("File", "qryQueueTiteReview")
