...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_ForeclosurePrint"
...
Private Sub chComplianceAffidavit_Click()
If chComplianceAffidavit = True Then
    If Me.ClientID = 385 And Me.State <> "MD" Then
    MsgBox ("This is not MD state")
   chComplianceAffidavit = False
    Exit Sub
...
Private Sub chLossMitFinal_Click()
...
Private Sub chLossMitPrelim_Click()
...
Private Sub chLostNoteAffidavit_Click()
...
Private Sub chNoteOwnership_Click()
...
End Sub
Private Sub ChAffiOfLienInstru_Click()
If ChAffiOfLienInstru Then
SumD = SumD + 1
Else
SumD = SumD - 1
End If
...
If chDOAAffidavit Then
    If PrintTo <> -1 Then
    Call DoReport("Deed of Appointment Affidavit", PrintTo)
    Else
        If Me.State = "MD" Then
        Call doc_SOTAffidavitMD(EMailStatus <> 1)
        Else
        Call DoReport("Deed of Appointment Affidavit", PrintTo)
        End If
    End If
...
'If ReportName = "Deed of Appointment Affidavit" And Forms!foreclosuredetails!State = "MD" Then
'            Call doc_SOTAffidavitMD(EMailStatus <> 1)
'        End If
...
If Me.chBaileeLetter Then 'And Forms![case list]!ClientID = 523 Then
...
If chSOD2 Then
    ...
    Select Case Forms![Case List]![ClientID]
        ...
        Call DoReport("Nationstar Cover Sheet", PrintTo)
        'prntTo = PrintTo
        Call AddStatmentDebt(FileNumber, PrintTo)
        'Mei 9/22/15
           ' Call DoReport("Nationstar Cover Sheet", PrintTo)
        ...
        '    If rstNS.EOF Then
        '    Description1 = "Interest Amount     "
        '    Description2 = "Interest [if adjustable rate]"
        '    Description3 = "Total Late Charges"
        '    Description4 = "Property Tax Advances"
        '
        '    With rstNS
        '        .AddNew
        '        !FileNumber = FileNumber
...
If chComplianceAffidavit Then
    ...
    Else
        ...
        Case 466 'SELECT
            ...
            DoCmd.OpenForm "Print Statement of Debt", , , "ForeclosureID=" & Forms!foreclosuredetails!ForeclosureID, , , PrintTo
            rstSelect.Close
        Case 385 'Nationstar   'Mei 9/22/15
               'Call AddStatmentDebt(FileNumber, PrintTo)
               '     prntTo = PrintTo
                    DoCmd.OpenForm "Prior Servicer", , , , , , PrintTo
...
If Me.State = "MD" Then
    ...
    Call ChAffiOfLienInstruFunction("Loss Mitigation Preliminary Nation Star", PrintTo)
...
If ChAffiOfLienInstru Then
Call ChAffiOfLienInstruFunction("Affidavit of Lien Instrument NationStar", PrintTo)
End If
...
If chHUDOccLabels Then Call chHUDOccLablesFunction
...
If chHUDOcc Then
    ...
    Call StartDoc(TemplatePath & "huddoc New Attach.pdf")      'Mei Attach new HUD doc here 10/13/15
...
If chDismissCase Then
Call DoReport("Dismiss Case", PrintTo)
Call UpdateDismiss
...
End Sub
Public Sub ChAffiOfLienInstruFunction(ReportName As String, PrintTo As Integer)

If Me.State = "MD" And Me.ClientID = 385 Then
...
Public Sub chHUDOccLablesFunction()
Dim i As Integer
Dim sql As String
...
Public Sub AddStatmentDebt(FileNumber As Long, PrintTo As Integer) ' old name 'tempp..
 Dim rstNS As Recordset
 Dim Desc1, desc2, desc3, desc4, desc5, desc6, desc7 As String
            Set rstNS = CurrentDb.OpenRecordset("Select * from statementofDebt where Filenumber=" & FileNumber, dbOpenDynaset, dbSeeChanges)
    If rstNS.EOF Then
    Desc1 = "Interest Amount     "
    desc2 = "Interest [if adjustable rate]"
    desc3 = "Total Late Charges"
    desc4 = "Property Tax Advances"
...
End Sub
Private Sub UpdateDismiss()
If MsgBox("Do you want to update Rosie Dismiss sent?", vbYesNo) = vbYes Then
   Forms!foreclosuredetails!DismissalSent = Format(InputBox("Please Enter Dismissal Sent date", "Dismissal Sent date"), "mm/dd/yyyy")
 '  Forms!Foreclosuredetails!DismissalDate = Format(Inputbox("Please Dismissal Date date", "dismissal Sent dates Entry"), "mm/dd/yyyy")
    DoCmd.SetWarnings False
    strSQLJournal = "Insert into Journal (FileNumber,JournalDate,Who,Info,Color) Values(Forms!ForeclosureDetails!FileNumber,Now,GetFullName(),'" & "Sent Dismissal to Court" & "',1 )"
    DoCmd.RunSQL strSQLJournal
    DoCmd.SetWarnings True
End If

End Sub
Private Sub ComDraft_Click()
If SumD > 1 Then
MsgBox ("Choose one only")
Exit Sub
Else
PrintDocs (-5)
End If
...
If (Not IsNull(FairDebtDispute) And IsNull(FairDebtVerified)) Then
  ...
  ChAffiOfLienInstru.Enabled = False
...
If Me.State = "VA" Then
  Attorney.RowSource = "SELECT Staff.ID, Staff.Name & ', ' & [CommonWealthTitle] AS CWRep " & _
                       "FROM Staff " & _
                       "WHERE  ((staff.active = true) And (Staff.Trustee =True) And [CommonWealthTitle]<> """") " & _
...
If OpenArgs = "docketing" Then
!chWarranty.Visible = False
!chLandInstruments.Visible = False
!chBaltCityIntake.Visible = False
!chLine.Visible = False
!ChCertOfCompliance.Visible = False
!chReturnDocs.Visible = False
!chCreateLabel.Visible = False
...
!ChAffiOfLienInstru.Visible = False
...
If Not PrivPrintPostSale Then chDismissCase.Enabled = False
End Sub
...
'    If Me.State = "MD" And Me.ClientID = 385 Then
'
'        DoCmd.OpenForm "PrintClientDoc", , , "FileNumber=" & Forms!ForeclosureDetails!FileNumber, , acDialog, "Deed of Appointment MD_Nationstar|Deed of Appointment MD_Nationstar|" & PrintTo
'
'     End If
...
'         DoCmd.OpenForm "PrintClientDoc", , , "FileNumber=" & Forms!foreclosuredetails!FileNumber, , acDialog, "Substitution of Trustee|Deed of Appointment SPLS VA|" & PrintTo
         DoCmd.OpenForm "PrintClientDoc", , , "FileNumber=" & Forms!foreclosuredetails!FileNumber, , acDialog, "Substitution of Trustee|AssignmentSOT VA Specialized|" & PrintTo
   ...
   ElseIf Me.JurisdictionID = 153 And Me.ClientID <> 385 Then  'ACCOMACK
   ...
   'added on 6/17/15
   If Me.ClientID = 385 Then
       DoCmd.OpenForm "PrintClientDoc", , , "FileNumber=" & Forms!foreclosuredetails!FileNumber, , acDialog, "Substitution of Trustee|Deed of Appointment VA_NationStar|" & PrintTo
    Else
