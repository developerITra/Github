...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queDemand"
...
Private Sub cmdExcel_Click()
    ...
    DoCmd.OutputTo acOutputQuery, "qryQueueDemand", acFormatXLS, TemplatePath & "Demand Queue info.xlt", True
...
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryQueueDemand_P", dbOpenDynaset)
...
.Edit
!DemandUser = StaffID
!DemandLastEdited = Date
.Update
End With
Set rstqueue = Nothing

Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryQueueDemand_P order by Date", dbOpenDynaset)
    If rstqueue.EOF Then
    cntr = 0
    Else
    rstqueue.MoveLast
...
Dim rstqueue As Recordset, cntr As Integer
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryQueueDemand", dbOpenDynaset)
 If rstqueue.EOF Then
 cntr = 0
 Else
 rstqueue.MoveLast
