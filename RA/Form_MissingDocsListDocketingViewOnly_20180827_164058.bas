...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_MissingDocsListDocketingViewOnly"
...
Set rstdocs = CurrentDb.OpenRecordset("Select * FROM DocketingDocsNeeded where filenumber=" & FileNbr & " AND DOCID=" & lstFiles.Value, dbOpenDynaset, dbSeeChanges)
...
Set rstdocs = CurrentDb.OpenRecordset("Select * FROM DocketingDocsNeeded where filenumber=" & FileNbr, dbOpenDynaset, dbSeeChanges)
...
'!JournalDate = Now
'!Who = GetFullName
'!Info = DocName & "was manually removed from the Docketing waiting list of outstanding items"
'.Update
'.Close
'End With

DoCmd.SetWarnings False
strinfo = DocName & "was manually removed from the Docketing waiting list of outstanding items"
