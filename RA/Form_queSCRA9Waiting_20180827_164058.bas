...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queSCRA9Waiting"
...
OpenCase lstFiles
Set rstqueue = CurrentDb.OpenRecordset("select * from qryQueueSCRAEVallWaiting where filenumber=" & lstFiles, dbOpenDynaset, dbSeeChanges)
...
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryQueueSCRAEVallWaiting", dbOpenDynaset, dbSeeChanges)
