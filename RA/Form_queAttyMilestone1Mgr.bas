...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queAttyMilestone1Mgr"
...
Dim cntr As Integer
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryqueueAttyMilestone1Mgr", dbOpenDynaset, dbSeeChanges)
...
'Set rstJnl = CurrentDb.OpenRecordset("Select * FROM journal where filenumber=" & lstFiles, dbOpenDynaset, dbSeeChanges)
    ...
    strinfo = "Fair Debt Review- Manager corrections made:  " & mgrInput
    strinfo = Replace(strinfo, "'", "''")
    strSQLJournal = "Insert into Journal (FileNumber,JournalDate,Who,Info,Color) Values(Forms!queAttyMilestone1Mgr!lstFiles,Now,GetFullName(),'" & strinfo & "',1 )"
...
'!JournalDate = Now
'!Who = GetFullName
'!Info = "Fair Debt Review- Manager corrections made:  " & mgrInput
'!Color = 1
'.Update
'End With
Set rstJnl = Nothing
