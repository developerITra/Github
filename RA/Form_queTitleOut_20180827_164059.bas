...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queTitleOut"
...
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryQueueTitleOut", dbOpenDynaset, dbSeeChanges)
...
Dim rstqueue As Recordset
AddToList (lstFiles)
TitleOutstandingCallFromQueue lstFiles
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM wizardqueuestats where filenumber=" & lstFiles & " AND current=true", dbOpenDynaset, dbSeeChanges)

With rstqueue
.Edit
!TitleOutLastReviewBy = GetStaffID
!TitleOutLastReview = Date
.Update
End With
Set rstqueue = Nothing
End Sub
...
Set rstqueue = CurrentDb.OpenRecordset("select count(*)as ct from qryQueueTitleOut")
