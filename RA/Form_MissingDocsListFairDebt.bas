...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_MissingDocsListFairDebt"
...
Set rstdocs = CurrentDb.OpenRecordset("Select * FROM FairDebtDocsNeeded where filenumber=" & FileNbr & " AND DOCID=" & lstFiles.Value, dbOpenDynaset, dbSeeChanges)
...
Set rstdocs = CurrentDb.OpenRecordset("Select * FROM FairDebtDocsNeeded where filenumber=" & FileNbr, dbOpenDynaset, dbSeeChanges)
