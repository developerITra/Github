VERSION 1.0 CLASS
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_queTitelOrder"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = True
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Compare Database
Option Explicit

Private Sub Fnumber_AfterUpdate()
If Not IsNull(Fnumber) Then
Dim Value As String
Dim blnFound As Boolean
blnFound = False
Dim J As Integer
Dim A As Integer
For J = 0 To lstFiles.ListCount - 1
   Value = lstFiles.Column(0, J)
   If InStr(Value, Fnumber.Value) Then
        blnFound = True
         A = J
        Me.lstFiles.Selected(A) = True
    Exit For
    End If
Next J

If Not blnFound Then MsgBox ("File not in the queue.")
lstFiles.SetFocus
End If
End Sub

Private Sub Fnumber_DblClick(Cancel As Integer)
Fnumber.Value = Null

End Sub

Private Sub lstFiles_GotFocus()
cmdViewItems.Enabled = True
End Sub

Private Sub cmdViewItems_Click()
DoCmd.OpenForm "MissingTitleOrderViewOnly"
Forms!MissingTitleOrderViewOnly!FileNbr = lstFiles
End Sub
Private Sub cmdOK_Click()
Dim rstqueue As Recordset

If IsNull(lstFiles) Then
MsgBox ("Please select File")
Exit Sub
End If

On Error GoTo Err_cmdOK_Click
''heresarab
Dim Trstqueue As Recordset
Set Trstqueue = CurrentDb.OpenRecordset("Select * FROM TitleOrderFinal where File=" & lstFiles, dbOpenDynaset, dbSeeChanges)

With Trstqueue
.Edit
![Last Review] = Date
![Last Review by] = GetFullName()
.Update
End With

Set Trstqueue = Nothing

Dim cntr As Integer

'Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryTitle_All", dbOpenDynaset, dbSeeChanges)
Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryTitle_All", dbOpenDynaset, dbSeeChanges)
If rstqueue.EOF Then
    cntr = 0
    Else
    rstqueue.MoveLast
    cntr = rstqueue.RecordCount
End If
Set rstqueue = Nothing
'Set rstqueue = CurrentDb.OpenRecordset("qryTitle_All")
'Do Until rstqueue.EOF
'cntr = cntr + 1
'rstqueue.MoveNext
'Loop
'QueueCount = cntr
'Set rstqueue = Nothing


'Set rstqueue = CurrentDb.OpenRecordset("Select * FROM wizardqueuestats where filenumber=" & lstFiles & " and current=true", dbOpenDynaset, dbSeeChanges)
'
'With rstqueue
'.Edit
'!TitleLastRevieDate = Date
'!TitleLastReiveUser = GetStaffID
'.Update
'End With
'
'Set rstqueue = Nothing

TitleOrderCallFromQueue lstFiles

Exit_cmdOK_Click:
    Exit Sub

Err_cmdOK_Click:
    MsgBox Err.Description
    Resume Exit_cmdOK_Click
    
End Sub

Private Sub cmdCancel_Click()

On Error GoTo Err_cmdCancel_Click


DoCmd.Close

Exit_cmdCancel_Click:
    Exit Sub

Err_cmdCancel_Click:
    MsgBox Err.Description
    Resume Exit_cmdCancel_Click
    
End Sub

Private Sub cmdRefresh_Click()

With DoCmd
.SetWarnings False
.OpenQuery "TitleOrderUpdatInsert"
.SetWarnings True
End With

Me!lstFiles.Requery
Me.Requery

Dim rstqueue As Recordset, cntr As Integer


Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryTitle_All", dbOpenDynaset, dbSeeChanges)
Do Until rstqueue.EOF
cntr = cntr + 1
rstqueue.MoveNext
Loop
QueueCount = cntr
Set rstqueue = Nothing


'Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryTitle_All", dbOpenDynaset, dbSeeChanges)
'If rstqueue.EOF Then
'    cntr = 0
'    Else
'    rstqueue.MoveLast
'    cntr = rstqueue.RecordCount
'End If
'Set rstqueue = Nothing
End Sub

Private Sub lstFiles_DblClick(Cancel As Integer)
Dim rstqueue As Recordset
Dim cntr As Integer
AddToList (lstFiles)
Dim Trstqueue As Recordset
''heresarab
Set Trstqueue = CurrentDb.OpenRecordset("Select * FROM TitleOrderFinal where File=" & lstFiles, dbOpenDynaset, dbSeeChanges)

With Trstqueue
.Edit
![Last Review] = Date
![Last Review by] = GetFullName()
.Update
End With

Set Trstqueue = Nothing

'Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryTitle_All", dbOpenDynaset, dbSeeChanges)
'If rstqueue.EOF Then
'    cntr = 0
'    Else
'    rstqueue.MoveLast
'    cntr = rstqueue.RecordCount
'End If
'Set rstqueue = Nothing


TitleOrderCallFromQueue lstFiles
End Sub
Private Sub Form_Open(Cancel As Integer)
Dim rstqueue As Recordset, cntr As Integer
'Select COUNT(Supplier_ID) from suppliers
'Set rstqueue = CurrentDb.OpenRecordset("Select * FROM qryTitle_All", dbOpenDynaset, dbSeeChanges)

'Set rstqueue = CurrentDb.OpenRecordset("qryTitle_All")

'Do Until rstqueue.EOF
'cntr = cntr + 1
'rstqueue.MoveNext
'Loop
'QueueCount = cntr
'Set rstqueue = Nothing



'10/14/14

Set rstqueue = CurrentDb.OpenRecordset("select count(*) as ct from qryTitle_All")
If Not rstqueue.EOF Then
    rstqueue.MoveLast
    'QueueCount = rstqueue.RecordCount
    QueueCount = rstqueue!ct

Else
    QueueCount = 0
End If

rstqueue.Close
Set rstqueue = Nothing

End Sub
