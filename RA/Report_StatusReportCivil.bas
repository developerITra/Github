...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Report_StatusReportCivil"
...
Private Sub GroupFooter2_Format(Cancel As Integer, FormatCount As Integer)
  ...
  If (DCount("[Hearing]", "[CivHearings]", "FileNumber = " & [FileNumber]) = 0) Then
    Me.lblHearingDate.Visible = False
  Else
    Me.lblHearingDate.Visible = True
  End If
End Sub
